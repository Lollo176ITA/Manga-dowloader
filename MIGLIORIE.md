...
- [ ] **Test del `DownloadWorker` (cuore dell'app)** 🔎
  - Dove: [DownloadWorker.kt](android-app/app/src/main/java/com/lorenzo/mangadownloader/DownloadWorker.kt) — `doWork`, `enqueue`, retry/cancellazione, concorrenza con `Semaphore`/`Mutex`. Piano e CBZ sono già coperti da `DownloadPlanAndCbzTest` (`data/sources`): manca l'orchestrazione.
